    ...
    var body: some View {
        HStack {
            ...
            Button(action: { presented.toggle() }){
                Color(UIColor.cyan).overlay(
                    Image(systemName: "multiply").foregroundColor(Color(UIColor.gray))
                ).frame(width: 30, height: 30).cornerRadius(15)
            }.padding([.horizontal, .top]).padding(.bottom, 8)
        }
        if isConnected {
            HStack {
                Button("disconnect"){ bluetooth.disconnect() }.buttonStyle(PlainButtonStyle()).padding([.horizontal])
                Spacer()
            }
        }
